/**
 * ts.charts exposes the generic interface to rendering a chart for a
...
module ts {
  export module charts {
    ...
    export var current_provider : any
    export var interactive : boolean = false
    export const DEFAULT_PALETTE : string = 'spectrum6'

    export const perf = ts.perf('ts.charts')

    export const registry = new ts.registry.Registry<any>({
      name: 'chart-provider',
      process: function(data) {
        if (data.is_chart_provider)
          return data
        return ts.charts.provider(data)
      }
    })

    function get_renderer(fn_name, item) : (element, item, query) => void {
      let interactive = ts.charts.interactive
      if ((typeof(item) !== 'undefined') && (typeof(item.interactive) !== 'undefined'))
        interactive = item.interactive
      var renderer = interactive ? current_provider[fn_name] : ts.charts.graphite[fn_name]
     ...
     */
    export function simple_line_chart(element, item, query) {
     ...
     */
    export function standard_line_chart(element, item, query) {
     ...
     */
    export function simple_area_chart(element, item, query) {
     ...
     */
    export function stacked_area_chart(element, item, query) {
     ...
     */
    export function donut_chart(element, item, query) {
   ...
   /**
     ...
     */
    export function bar_chart(element, item, query) {
   ...
   /**
     ...
     */
    export function discrete_bar_chart(element, item, query) {
     ...
     *                      provider.
     */
    export function process_series(series, type?) {
      var processed = null
      if (type) {
        processed = ts.charts.registry.get(type).process_series(series)
      } else {
        processed = current_provider.process_series(series)
      }
     ...
     *                      provider.
     */
    export function process_data(data, type) {
      if (!data)
        return []
      if (data instanceof Array) {
        return data.map(function(series) {
          return process_series(series, type)
        })
      } else {
    ...
    export const util =
      (function () {
        ...
        self.get_palette = function(name) {
          ...
          return palette || self.colors[ts.charts.DEFAULT_PALETTE];
        }
        ...
        self._color_function = function(palette_name) {
          ...
          if (!palette) {
            palette = self.colors[ts.charts.DEFAULT_PALETTE];
          }
          return function(d, i) {
