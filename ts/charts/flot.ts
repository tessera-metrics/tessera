  ...
  export module charts {
    ...
    export const flot =
      (function() {
        ...
        var log = ts.log.logger('ts.charts.flot')
        ...
        function get_default_options() {
          var theme_colors = ts.charts.util.get_colors()
          var default_options = {
            colors: ts.charts.util.get_palette(),
            downsample: false,
            series: {
              lines: {
                show: true,
        ...
        function setup_plugins(container, context) {
          ...
          $(container).bind("multihighlighted", function(event, pos, items) {
            if ( !items )
              return
            var is_percent = context.item.stack_mode && (context.item.stack_mode === ts.charts.StackMode.PERCENT)
        ...
        self.stacked_area_chart = function(e, item, query) {
          ...
          if (item.stack_mode === ts.charts.StackMode.PERCENT) {
          ...
          } else if (item.stack_mode == ts.charts.StackMode.STREAM) {
            options.series.stackD3.offset = 'wiggle'
          } else if (item.stack_mode == ts.charts.StackMode.NONE) {
